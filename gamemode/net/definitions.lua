...
NetworkDefinitions = {
    ...
    CSRequestCode = {
        ...
        HERO_ABILITY_CD_GET = 7, -- Client asks for cooldown of ability [IN: ability name / identifier (string), OUT: ability name; time cooldown is ready next, 0 for already ready (number)]
        ABILITY_ITEM_LINKS_GET = 10 -- Client asks for items
    },
    -- Server -> client
    SCRequestCode = {
        UNKNOWN = 0,
        ...
        DAMAGE_INDICATOR = 9 -- Server notifying client about damage it should draw [IN: damage amount (number); x (number); y (number); z (number); damage type (number)]
    },
